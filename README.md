...
4. Настройте лимиты хранения артефактов, детекцию инъекций («Детекция инъекций», «Отслеживать цели инъекций») и при необходимости включите запись PCAPNG.
...
7. ExeScope зафиксирует появление процесса, перейдет в «Запись событий» и начнет выводить телеметрию на вкладках интерфейса (включая вкладку «Инъекции» и метки `[INJECTION TARGET]` в дереве процессов).
